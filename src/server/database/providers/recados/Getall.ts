...
export const getAll = async (page: number, limit: number, filter: string, id = 0): Promise<IRecadosWithRecados[] | Error> => {
  try {
    let query = Knex(ETableNames.recados).select('*').offset((page - 1) * limit).limit(limit);
    ...
    const resultWithRecados = result.map(user => {
      const userRecados = recados.filter(recado => recado.recado_id === user.id);
      return {
        ...user,
        usariosRec: userRecados
      };
    });
